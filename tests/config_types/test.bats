...
@test "cannot deploy project with invalid annotations" {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME/test-cases/invalid-annotations
	assert_failure
...
@test "cannot deploy project with invalid env" {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME/test-cases/invalid-env
	assert_failure
...
@test "cannot deploy project with invalid parameters" {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME/test-cases/invalid-parameters
	assert_failure
...
@test "cannot deploy project with invalid top-level parameter " {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME/test-cases/invalid-parameters-toplevel
	assert_failure
