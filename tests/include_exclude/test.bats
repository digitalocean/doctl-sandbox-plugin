...
@test "deploy project excluding folder " {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME --exclude test-ie-admin
	assert_success
...
@test "deploy project including folder " {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME --include test-ie-printer/notify
	assert_success
...
@test "deploy project with multiple include/excludes" {
	run $DOCTL sbx deploy $BATS_TEST_DIRNAME --include test-ie-admin/,test-ie-printer --exclude test-ie-printer/notify,test-ie-printer/update
	assert_success
